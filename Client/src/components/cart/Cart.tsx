...
const Cart = () => {
  ...
  const { cartItems, initLength, total }: { cartItems: any; initLength: number; total: number } =
    useSelector((state: RootState) => state.shop);
  const { total: changeTotal, length: changeLength } = useSelector(
    (state: RootState) => state.cart
  );
  const { userData }: { userData: any } = useSelector((state: RootState) => state.shop);
  ...
  const handleIdChecked = (id: string, isCheck: boolean) => {
    dispatch(getCartTotalFetch({ accessToken, id, isCheck }));
    setTotalCost(changeTotal);
  };

  const handleClickQty = (id: string, isCheck: boolean) => {
    dispatch(getCartTotalFetch({ accessToken, id, isCheck }));
    setTotalCost(changeTotal);
  };
    ...
    swalWithBootstrapButtons
      ...
      .then((result) => {
        if (result.isConfirmed) {
          ...
          setCartQuantity(0);
          setTotalCost(0);
          swalWithBootstrapButtons.fire('Deleted!', 'Cart Items has been deleted.', 'success');
        } else if (result.dismiss === Swal.DismissReason.cancel) {
          swalWithBootstrapButtons.fire('Cancelled', 'Cart Item is safe :)', 'error');
        }
      });
  };
  useEffect(() => {
  ...
  const deleteItem = (isDelete: boolean) => {
    if (isDelete) {
      setCartQuantity(0);
      setTotalCost(0);
    }
          ...
          <div className="cart__left left">
            <div className="left__bar d-flex justify-content-between align-items-center">
              <div className="left__check d-flex justify-content-between align-items-center">
                <p className="mb-0 ml-3">{cartItems?.length} mặt hàng</p>
              </div>
              <div
                className="left__button--delete d-flex justify-content-between align-items-center"
                onClick={deleteAll}
              >
                <i className="fas fa-trash-alt"></i>
                <p className="m-0 ml-1">Xóa</p>
              </div>
            </div>
              ...
              {cartItems?.length !== 0 ? (
                cartItems?.map((item: any) => (
                  <CartItem
                    ...
                    handleIdChecked={handleIdChecked}
                    isDelete={deleteItem}
                ...
                <img
                  src="/assets/img/empty-cart-png-Transparent-Images.png"
          ...
          <div className="cart__right right">
            ...
            <div className="right__mid">
              ...
              <div className="d-flex justify-content-between align-items-center">
                ...
                <div className="choose-coupon">Chọn phương thức</div>
              </div>
              <div className="d-flex justify-content-between align-items-center mt-3 mb-3">
                <p className="m-0">ShopeeFood Voucher</p>
                <div className="choose-coupon">Chọn hoặc nhập mã</div>
              </div>
              <div className="d-flex justify-content-between align-items-center">
                <p className="m-0">Tạm tính ({cartQuantity} mặt hàng)</p>
                <p className="m-0">
                  <FormattedNumber value={totalCost} style="currency" currency="VND" />
                </p>
              </div>
            </div>
            <div className="right__footer">
              ...
              <button onClick={handleClick}>Xác nhận giỏ hàng ({cartQuantity} mặt hàng)</button>
            </div>
          </div>
        </div>
      </div>
