...
const PurchaseAll = () => {
  ...
  useEffect(() => {
    const callApi = async () => {
      try {
        if (accessToken) {
          const getOrder = await axios.post(`${URL}/purchase/get-order-info/type`, {
            type,
            accessToken,
          });
  ...
  const handleReBuy = async (product_IDS: any) => {
  ...
  return (
    <div className="purchase-list">
      {data?.map((d: any) => {
        const total = d.products.reduce((a: number, b: any) => a + b.cost * b.qty, 0);
        return (
          <div className="items-purchase" key={d._id}>
            <div className="item" key={d._id}>
              <div className="item__header d-flex justify-content-end align-items-center">
                ...
                <p>
                  {(d.status === 1 || d.status === 2) && 'Đang xử lý'}
                  {d.status === 3 && 'Đã giao'}
                </p>
                <p>
                  <FormattedDate value={d.updatedAt} year="numeric" month="long" day="2-digit" />
                </p>
                <p>
                  <FormattedTime value={d.updatedAt} />
                </p>
              </div>
              {d?.products?.map((p: any) => {
                return (
                  <div
                    className="item__mid d-flex justify-content-between align-items-center"
                    key={p.product_ID}
                  >
                    ...
                    <p className="price">
                      <FormattedNumber value={p.cost} style="currency" currency="VND" />
                    </p>
                  </div>
                );
              })}
              <div className="d-flex justify-content-end align-items-center mt-2 mb-2">
                ...
                <p className="mb-0 total-price">
                  <FormattedNumber value={total} style="currency" currency="VND" />
                </p>
              </div>
              <div className="item__bottom d-flex justify-content-end align-items-center">
                <button onClick={() => handleReBuy(d?.products)} className="btn btn-buy">
                  Mua lại
                </button>
