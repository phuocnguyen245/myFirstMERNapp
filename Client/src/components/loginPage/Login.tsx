...
import { getUserInfoFetch } from './loginSlice';
...
const Login = () => {
  ...
  const { user }: { user: any } = useSelector((state: RootState) => state.loginpage);

  const { status } = useSelector((state: RootState) => state.loginpage);

  const dispatch = useDispatch();

  // get Token and save to cookie
  const token = useSelector((state: RootState) => state.loginpage.accessToken);
        ...
        <div className="login-container ">
          ...
          {(errors?.username || errors?.password) && (
            <div className="login-alert">
              {errors?.username && (
                <span className="block" style={{ color: '#a94442' }}>
          ...
          <form onSubmit={handleSubmit(submitForm)} className="form" id="form-1">
            ...
            <div className="form-group row justify-content-between">
              <div className="row align-items-center">
                ...
                <label htmlFor="checked-login" className="m-0">
          ...
          <div className="login-policy">
            <p>
              Chúng tôi không sử dụng thông tin của bạn với bất kỳ mục đích nào. Bằng cách đăng nhập
              hoặc đăng ký, bạn đồng ý với{' '}
              <a href="/#">
